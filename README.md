...
2. 打开 `scripts`，双击 **`Scan-360.cmd`**。只读扫描结束后会自动弹出项目列表；默认一个都不选。
3. 绿色 `Confirmed` 项可逐项勾选；黄色 `ReviewOnly`、离线系统项目和受保护资料不能在这里删除。也可以点击“只保留报告”直接退出。
4. 点击“删除所选项目”后还会再次确认并触发 Windows 管理员授权；未勾选目标会保留。

**看到选择窗口就表示扫描完成。** `Confirmed` 表示有匹配证据，仍需你亲自勾选；`ReviewOnly` 表示仅供复核。看不懂时点击“只保留报告”。扫描和退出都不会删除任何内容。
...
如果不能操作终端，不要声称已经扫描或删除。请让我下载并完整解压仓库 ZIP，双击 scripts\Scan-360.cmd；弹窗默认一个都不选，只允许绿色 Confirmed。先点“只保留报告”，原始 JSON 留在本机；只向云端提供另做的脱敏副本或节选。
只有我看懂结果并明确决定后，才指导我重新 Scan，只勾选批准的绿色项目并点击“删除所选项目”；未勾选和黄色项目必须保留，不能使用脱敏副本删除。
...
| 默认不选、逐项决定 | 双击 Scan 后弹出复选框；只有绿色 `Confirmed` 可选，未勾选和黄色项目都会保留 |
...
4. 等待弹出选择窗口；逐项阅读名称、目标和判断依据。默认没有任何勾选。
5. 不想删除时点击“只保留报告”；要清理时只勾选绿色 `Confirmed`，再点击“删除所选项目”。

扫描本身不会删除任何东西，也不需要管理员权限。点击删除后还会出现第二次确认；选择“否”仍不会删除。

### 2. 旧版/高级整份报告入口

推荐优先使用上面的逐项勾选窗口。只有在明确要处理同一份 Scan 报告中的全部可删除目标时，才使用这个备用入口：
...
删除是永久操作，不会进入回收站。逐项入口会把所选稳定 ID 和原始 Scan JSON 的 SHA-256 一并绑定；报告被修改、所选目标变化或消失时，会在任何删除发生前中止。提权后新出现但未获批的目标只报告、不删除。
